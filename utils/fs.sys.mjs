export class FileSystem {
  ...
  static getFileURIForFile(aEntry, type) {
    ...
    throw ResultError.fromKind(FileSystem.ERROR_KIND_INVALID_ARGUMENT, {
  ...
  static convertChromeURIToFileURI(aURI) {
    ...
    return registry.convertChromeURL(
      aURI instanceof Ci.nsIURI ? aURI : Services.io.newURI(aURI)
    );
  }
  ...
  static getResourceDir() {
    return FileSystemResult.fromNsIFile(
      FileSystem.RESOURCE_URI.QueryInterface(Ci.nsIFileURL).file
    );
  }

  static getScriptDir() {
    return FileSystemResult.fromNsIFile(
      FileSystem.SCRIPT_URI.QueryInterface(Ci.nsIFileURL).file
    );
  }

  static getStyleDir() {
    return FileSystemResult.fromNsIFile(
      FileSystem.STYLE_URI.QueryInterface(Ci.nsIFileURL).file
    );
  }
  ...
  static getEntry(aFilename, options = {}) {
    if (aFilename instanceof Ci.nsIURI) {
      if (aFilename.scheme === "chrome") {
        return FileSystemResult.fromNsIFile(
          FileSystem.convertChromeURIToFileURI(aFilename).QueryInterface(
            Ci.nsIFileURL
          ).file
        );
      }
      if (aFilename.scheme === "file") {
        return FileSystemResult.fromNsIFile(
          aFilename.QueryInterface(Ci.nsIFileURL).file
        );
      }
      throw new Error("unsupported nsIURI conversion");
    }
    return FileSystem.#getEntryFromString(
      aFilename,
      options.baseDirectory || FileSystem.RESOURCE_URI
    );
  }
  static readNSIFileSyncUncheckedWithOptions(aFile, options) {
    ...
    try {
      ...
      stream.close();
      return FileSystemResult.fromErrorKind(
        FileSystem.ERROR_KIND_NOT_READABLE,
        { cause: e, filename: aFile.leafName }
      );
    }
    let rv = {
      content: "",
      path: FileSystem.getFileURIForFile(aFile, FileSystem.RESULT_FILE),
    };
    let data = {};
  ...
  static readFileSync(aFile, options = {}) {
    if (typeof aFile === "string") {
      const fsResult = FileSystem.#getEntryFromString(
        aFile,
        FileSystem.RESOURCE_URI
      );
      if (fsResult.isFile()) {
        return FileSystem.readNSIFileSyncUncheckedWithOptions(
          fsResult.entry(),
          options
        );
      }
      return fsResult.isError()
        ? fsResult
        : FileSystemResult.fromErrorKind(FileSystem.ERROR_KIND_NOT_FILE, {
            topic: aFile,
          });
    }
  ...
  static #appendToBaseURI(aPath, aFileURI) {
    ...
    let baseParts = PathUtils.split(
      baseURI.QueryInterface(Ci.nsIFileURL).file.path
    );
  ...
  static createFileURI(fileName) {
    ...
    return FileSystem.convertChromeURIToFileURI(
...
class FileSystemResult {
  ...
  content(replaceNewlines) {
    ...
    throw ResultError.fromKind(FileSystem.ERROR_KIND_NOT_CONTENT, {
  ...
  entry() {
    ...
    throw ResultError.fromKind(
      ...
      FileSystemResult.#generateErrorInfo(this)
    );
  }
  error() {
  ...
  readSync() {
    if (!this.isFile()) {
      throw ResultError.fromKind(
        FileSystem.ERROR_KIND_NOT_FILE,
        FileSystemResult.#generateErrorInfo(this)
      );
    }
    return FileSystem.readNSIFileSyncUncheckedWithOptions(
  ...
  entries() {
    if (!this.isDirectory()) {
      throw ResultError.fromKind(
        FileSystem.ERROR_KIND_NOT_DIRECTORY,
        FileSystemResult.#generateErrorInfo(this)
      );
    }
  ...
  static fromErrorKind(aKind, aErrorDescription) {
    return new FileSystemResult(
      ResultError.fromKind(aKind, aErrorDescription),
  ...
  static fromNsIFile(entry) {
    ...
    return FileSystemResult.fromErrorKind(
      FileSystem.ERROR_KIND_UNKNOWN_RESULT,
      { topic: entry.leafName }
    );
  }
}
