...
ChromeUtils.defineLazyGetter(SharedGlobal, "widgetCallbacks", () => {
  return new Map();
});
const lazy = {
...
export class Hotkey {
  ...
  constructor(hotkeyDetails, commandDetails) {
    ...
    this.#matchingSelector = `key[modifiers="${hotkeyDetails.modifiers}"][${
      hotkeyDetails.key
        ? 'key="' + hotkeyDetails.key
        : 'keycode="' + hotkeyDetails.keycode
    }"]`;
  }
  ...
  static #createCommand(doc, details) {
    ...
    let command = createElement(doc, "command", {
      id: details.id,
    ...
    onBuild: function (aDocument) {
      ...
      if (typeof callback === "function") {
        toolbaritem.setAttribute(
          ...
          }UC_API.SharedStorage.widgetCallbacks.get(this.id)(event,window)`
        );
      }
...
export function toggleScript(el) {
  ...
  let script = name.endsWith("js") ? getScriptData(name) : getStyleData(name);
  if (!script) {
    return null;
  }
